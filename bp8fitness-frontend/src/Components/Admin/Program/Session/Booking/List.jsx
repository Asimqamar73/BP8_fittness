...
import { faDumbbell, faGauge, faSearch, faEye, faPencil, faTrashCan, faPlus, faArrowRight, faTable, faArrowUpRightFromSquare, faFilter, faRefresh, faArchive } from '@fortawesome/free-solid-svg-icons';
...
function AdminWorkoutProgramSessionBookingList() {
    ////
    //// URL Parameters.
    ////
    ...
    const fetchList = (cur, limit, keywords, b, wp, wps, t, sdt) => {
        setFetching(true);
        setErrors({});
        ...
        if (wps !== undefined && wps !== null && wps !== "") {
            params.set("workout_session_id", wps);
        }
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            fetchList(currentCursor, pageSize, actualSearchText, branchID, workoutProgramID, sid, trainerID, startAt);
    ...
    }, [sid, currentCursor, pageSize, actualSearchText, branchID, workoutProgramID, trainerID, startAt]);
                ...
                <section class="section">
                    <nav class="breadcrumb" aria-label="breadcrumbs">
                        <ul>
                            ...
                            <li class="is-active"><Link aria-current="page"><FontAwesomeIcon className="fas" icon={faEye} />&nbsp;Schedule Detail (Bookings)</Link></li>
                        </ul>
                    </nav>
                    <nav class="box">
                        <div class={`modal ${selectedWorkoutSessionForDeletion ? 'is-active' : ''}`}>
                            ...
                            <div class="modal-card">
                                ...
                                <section class="modal-card-body">
                                    You are about to <b>cancel this members booking</b> and <b>archive</b> it; the booking will no longer appear on your dashboard. This action can be undone but you'll need to contact the system administrator. Are you sure you would like to continue?
                                </section>
                        ...
                        <div class="columns is-mobile">
                            <div class="column">
                                <h1 class="title is-4"><FontAwesomeIcon className="fas" icon={faEye} />&nbsp;Schedule Detail</h1>
                            </div>
                            <div class="column has-text-right">
                                <button onClick={()=>fetchList(currentCursor, pageSize, actualSearchText, branchID, workoutProgramID, sid, trainerID, startAt)} class="button is-small is-info" type="button">
                        ...
                        {isFetching
                            ...
                            : <>
                                <div class="tabs is-medium">
                                  <ul>
                                    <li>
                                        <Link to={`/admin/branch/${bid}/class/${pid}/session/${sid}`}>Detail</Link>
                                    </li>
                                    <li class="is-active">
                                ...
                                {listData && listData.results && (listData.results.length > 0 || previousCursors.length > 0)
                                    ?
                                    <>
                                        {currentWorkoutSession.attendeesCount >= currentWorkoutSession.attendeesLimit
                                            ?
                                            <article class="message is-success">
                                                <div class="message-body">
                                                    <strong>Schedule is at maximum capacity ({currentWorkoutSession.attendeesLimit})</strong>
                                                </div>
                                            </article>
                                            :
                                            <article class="message">
                                                <div class="message-body">
                                                    <strong>Schedule available capacity: {currentWorkoutSession.attendeesCount} / {currentWorkoutSession.attendeesLimit}</strong>
                                                </div>
                                            </article>
                                        }
                                                    ...
                                                    <table class="table is-fullwidth is-striped is-hoverable is-fullwidth">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            {listData && listData.results && listData.results.map(function(datum, i){
                                                                return <tr>
                                                                    <td data-label="Name">
                                                                        <Link to={`/admin/member/${datum.memberId}`} target="_blank" rel="noreferrer" class="">
                                                                            {datum.memberName}&nbsp;<FontAwesomeIcon className="fas" icon={faArrowUpRightFromSquare} />
                                                                        </Link>
                                                                    </td>
                                                                    <td data-label="Email">
                                                                        {datum.memberEmail
                                                                            ?
                                                                            <Link to={`mailto:${datum.memberEmail}`} target="_blank" rel="noreferrer" class="">
                                                                            {datum.memberEmail}
                                                                            </Link>
                                                                            :
                                                                            <>-</>
                                                                    ...
                                                                    <td data-label="Phone">
                                                                        {datum.memberPhone
                                                                            ?
                                                                            <Link to={`tel:${datum.memberPhone}`} target="_blank" rel="noreferrer" class="">{datum.memberPhone}</Link>
                                                                            :
                                                                            <>-</>
                                                                        }
                                                                    </td>
                                                                    ...
                                                                    <td class="is-actions-cell">
                                                                        <div class="buttons is-right">
                                                                            ...
                                                                            <Link to={`/admin/branch/${datum.branchId}/class/${datum.workoutProgramId}/session/${datum.workoutSessionId}/booking/${datum.id}`} class="button is-small is-primary" type="button">
                                                                                <FontAwesomeIcon className="mdi" icon={faEye} />&nbsp;View
                                                                            </Link>
                                                                            <Link to={`/admin/branch/${datum.branchId}/class/${datum.workoutProgramId}/session/${datum.workoutSessionId}/booking/${datum.id}/update`} class="button is-small is-warning" type="button">
                                    ...
                                    <section class="hero is-medium has-background-white-ter">
                                          <div class="hero-body">
                                            ...
                                            <p class="subtitle">
                                                No bookings. <b><Link to={`/admin/branch/${bid}/class/${pid}/session/${sid}/bookings/add`}>Click here&nbsp;<FontAwesomeIcon className="mdi" icon={faArrowRight} /></Link></b> to get started creating booking a member for this schedule.
                                            </p>
                                          </div>
                                    </section>
                                }
...
export default AdminWorkoutProgramSessionBookingList;
